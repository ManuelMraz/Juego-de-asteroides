...
void setup(){
 ...
 asteroidPics[0] = loadImage("Asteroide0.jpg"); // ingresamos la ruta de la imagen
 asteroidPics[1] = loadImage("Asteroide1.png");
 asteroidPics[2] = loadImage("Asteroide2.png");
 naves = loadImage("naves.jpg");
 frameRate(24);
 lives = 3; // DECLARAMOS LAS VIDAS QUE TENDREMOS EN EL JUEGO
 ...
 if( lives >= 0 && asteroids.size()>0){
   ...
   if(darkCounter < darkCounterLimit){
    ...
    rect(0,0,width,height);
     ...
     fill(255);
     if(stage > -1){
         text("Nivel " + (stage + 1) + " Completado", width/2-120, height/2);
     } else {
         text("Bienvenido Al Juego", width/2-80, height/2);
     }

     textFont(font, 15);
     fill(255);
     text("Has Click En La Pantalla Nivel=" + (stage + 2), width/2-100, height*.75);
...
class Ship{
 ...
 float drag = .9;
 float r = 15;
 PImage img = loadImage("naves.jpg");
